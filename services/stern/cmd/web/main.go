...
func InitializeWebRoutes(server *server.Server) {
	...
	server.Handle("GET /wiki", routes.WikiRedirect)
	server.Handle("GET /wiki/{$}", routes.WikiRedirect)
	server.Handle("GET /wiki/{language}", routes.WikiLanguageRedirect)
	server.Handle("GET /wiki/{language}/{$}", routes.WikiHome)
	server.Handle("GET /wiki/{language}/search", routes.WikiSearchRedirect)
	server.Handle("GET /wiki/{language}/search/{$}", routes.WikiSearch)
	...
	server.Handle("GET /beatmapsets/packs/{$}", routes.BeatmapPacks)
	server.Handle("GET /beatmapsets/{id}", routes.RedirectToBeatmapset)
	server.Handle("GET /beatmapsets/{setId}/discussion/", routes.RedirectToDiscussion)
	server.Handle("GET /beatmapsets/{setId}/discussion/{mapId}", routes.RedirectToDiscussion)
...
func main() {
	...
	stern := server.NewServer(app.Config.FrontendHost, app.Config.FrontendPort, "stern", app, engine, wikiService)
